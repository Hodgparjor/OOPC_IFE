...
Matrix& Matrix::operator+=(const Matrix& matrix)
{
	...
	for (size_t rowIndex = 0; rowIndex < this->getNumberOfRows(); rowIndex++) {
		for (size_t colIndex = 0; colIndex < this->getNumberOfColumns();
			 colIndex++) {
			this->data->elements[rowIndex][colIndex] +=
				matrix.data->elements[rowIndex][colIndex];
		}
	}
	return *this;
}
...
Matrix matrixMultiplication(const Matrix& matrix1, const Matrix& matrix2)
{
	...
	Matrix resultMatrix(matrix1.getNumberOfRows(),
						matrix2.getNumberOfColumns());
	for (size_t rowIndex = 0; rowIndex < resultMatrix.getNumberOfRows();
		 rowIndex++) {
		for (size_t colIndex1 = 0;
			 colIndex1 < resultMatrix.getNumberOfColumns(); colIndex1++) {
			for (size_t colIndex2 = 0; colIndex2 < matrix1.getNumberOfColumns();
				 colIndex2++) {
				resultMatrix.data->elements[rowIndex][colIndex1] +=
					matrix1.data->elements[rowIndex][colIndex2] *
					matrix2.data->elements[colIndex2][colIndex1];
			}
		}
	}
	return resultMatrix;
...
Matrix numberMultiplication(const Matrix& matrix, double number)
{
	...
	for (size_t rowIndex = 0; rowIndex < matrix.getNumberOfRows(); rowIndex++) {
		for (size_t colIndex = 0; colIndex < matrix.getNumberOfColumns();
			 colIndex++) {
			resultMatrix.data->elements[rowIndex][colIndex] =
				matrix.data->elements[rowIndex][colIndex] * number;
		}
	}
	return resultMatrix;
}
...
Matrix operator*(const Matrix& matrix1, const Matrix& matrix2)
{
	...
	else if (matrix2.getNumberOfColumns() == 1 &&
			 matrix2.getNumberOfReferences() == 1) {
...
bool operator==(const Matrix& matrix1, const Matrix& matrix2)
{
	...
	for (size_t rowIndex = 0; rowIndex < matrix1.getNumberOfRows();
		 rowIndex++) {
		for (size_t colIndex = 0; colIndex < matrix1.getNumberOfColumns();
			 colIndex++) {
			if (matrix1.data->elements[rowIndex][colIndex] !=
				matrix2.data->elements[rowIndex][colIndex]) {
				return false;
			}
		}
	}
...
{
	if (row < 0 || row >= this->data->numberOfRows || column < 0 ||
		column >= this->data->numberOfColumns) {
...
Matrix::MatrixData* Matrix::MatrixData::detach()
{
	...
	MatrixData* detachedData =
		new MatrixData(this->numberOfColumns, this->numberOfRows);
	for (size_t rowIndex = 0; rowIndex < this->numberOfRows; rowIndex++) {
		for (size_t colIndex = 0; colIndex < this->numberOfColumns;
			 colIndex++) {
			detachedData->elements[rowIndex][colIndex] =
				this->elements[rowIndex][colIndex];
		}
	}
...
MatrixReferenceHandler::MatrixReferenceHandler(Matrix* matrix, size_t row,
											   size_t column)
{
	this->matrix = matrix;
	this->row = row;
...
void MatrixReferenceHandler::operator=(const MatrixReferenceHandler& matrixElement)
{
	this->matrix->writing(this->row, this->column, matrixElement.matrix->reading(matrixElement.row, matrixElement.column));
}
